...
#  core_summary.sh     
#
#  This script invokes script core_countstat.sh  each
...
#  D. Stokes  04/2015     Moved to vertical structure package obscore_prep_post 
...
#    00Z counts
  ...
  $USHprepobs/core_countstat.sh $date core $numdays 00 ${MONTH} 

#    06Z counts

  $USHprepobs/core_countstat.sh $date core $numdays 06 ${MONTH}

#    12Z counts

  $USHprepobs/core_countstat.sh $date core $numdays 12 ${MONTH} 
 
#    18Z counts

  $USHprepobs/core_countstat.sh $date core $numdays 18 ${MONTH}


#   Execute monthly_avg
#  Compute average daily non-sat counts for the month ( output is in file tmp1 )

  export pgm=corecounts_ave
  . $DATA/prep_step

  export FORT10=core_${MONTH}_dumpstats.t00z
  export FORT11=core_${MONTH}_dumpstats.t06z
  export FORT12=core_${MONTH}_dumpstats.t12z
  export FORT13=core_${MONTH}_dumpstats.t18z
  export FORT50=tmp1

  $EXECprepobs/corecounts_ave 1>aa.out 2>aa.out
  export err=$?; err_chk
...
sed -e 's/= */=/g' tmp3 > ${MONTH}.core.outfile
echo "MONTH=${MONTH}" > ${MONTH}.htmlscript
echo "YEAR=$YEAR" >> ${MONTH}.htmlscript
cat ${MONTH}.core.outfile >> ${MONTH}.htmlscript
echo "cat <<EOF > ${MONTH}.html" >> ${MONTH}.htmlscript
cat $FIXprepobs/corecounts_html  >> ${MONTH}.htmlscript
...
# Combine 4 files ( 1 non-sat, 3 sat ) into 1
  ...
  $USHprepobs/corecounts_combine.sh                           
