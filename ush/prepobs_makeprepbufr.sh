...
#      (def=$PARMPREP/prepobs_nrlacqc.${modNET}.parm"), if NRLACQC=YES will
...
# 2017-06-10  D.C. Stokes -- Added logic to accommodate modNET=cfs.  Added
...
#     modNET        String indicating system network {either "gfs", "gdas",
#                   "cdas", "nam", "rap", "rtma" or "urma"}
#                   NOTE : modNET is changed to gdas in the parent Job script for
...
#                   program
#                   Default is "$PARMPREP/prepobs_prepdata.${modNET}.parm"
...
#                   if imported "modNET=cdas", default is
#                   "$HOMEprepobs/fix/prepobs_errtable.cdas";
#                   if imported "modNET=nam", default is
...
#                   Default is "$PARMSYND/syndat_syndata.${modNET}.parm"
...
#                   program
#                   Default is "$PARMPREP/prepobs_prevents.${modNET}.parm"
...
#                   program
#                   Default is "$PARMPREP/prepobs_prepacqc.${modNET}.parm"
...
#                   program
#                   Default is "$PARMPREP/prepobs_profcqc.${modNET}.parm"
...
#                   program
#                   Default is "$PARMPREP/prepobs_cqcbufr.${modNET}.parm"
...
#                   NOTE: If imported "modNET=cdas", default is
...
PRPC=${PRPC:-$PARMPREP/prepobs_prepdata.${modNET}.parm}
...
#  --------------------------------------------------------------------------
   ...
   [ $RUN = gfs -o $RUN = gdas -o $modNET = cfs ]  &&  qual_last=""
...
###cp ???????????? syndata_cond
   ...
   if [ "$SYN" = 'YES' ]; then
      ...
      if [ "$RELOCATION_HAS_RUN" != 'YES' -o "$modNET" = 'nam' ]; then

# If RELOCATION_HAS_RUN=NO or modNET=nam, always use original tcvitals file here
#  to ensure that SYNDATA will run if there are records in the original
#  tcvitals file (for modNET=nam, RELOCATION_HAS_RUN may be YES but previous
...
#  If RELOCATION_HAS_RUN=YES, modNET is not nam (currently meaning it is gfs or
