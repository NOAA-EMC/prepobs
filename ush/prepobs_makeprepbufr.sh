...
#  problem: adpsfc (surface land), adpupa (raob/pibal/recco) file, or uprair
#           (alt raob\pibal) file, or ALL, not found for requested time
#            - this is unacceptable; EXIT
...
BUFRLIST_all="uprair aircar aircft satwnd proflr vadwnd rassda adpupa adpsfc \
 sfcshp sfcbog msonet spssmi erscat qkswnd wdsatr ascatw rtovs atovs goesnd gpsipw"
 ...
 amongst all PREPOBS_PREPDATA runs is " $errPREPDATA
   ...
   if [ "$errPREPDATA" -le "$errPREPDATA_limit" -a $errPREPDATA -ne 1 ]; then
      ...
      if [ "$errPREPDATA" -eq '4' ]; then
         set +x
         echo
   echo "WARNING: PREPOBS_PREPDATA FOUND EITHER NO ADPUPA, NO UPRIAR, \
OR NO ADPSFC DATA"
