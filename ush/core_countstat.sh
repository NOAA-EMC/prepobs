...
#
#
#  This script collects core observational data counts from Dennis Keysers
#  status count files.
#
#  Usage: core_countstat.sh<start_date> <network> <num days> <hour> <month> [run]
#
#  History
...
#             num=`grep $type $TF | grep -vi Global | cut -c65-71`
  ...
  do
        ...
        then 
          ...
          mnemonic=`grep $type $FIXprepobs/corecounts_types | awk '{print $2}'`
          awk -f $FIXprepobs/corecounts_avg.awk $type var=$mnemonic >> ${net}_${month}_dumpstats.$CYCLE
        else
