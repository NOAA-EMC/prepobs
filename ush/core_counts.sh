...
#
    ...
    do
      ...
      INFILE=${COMINcore}${PDY}/${cycdir}/$COMPONENT/$RUN.${d_cyc}.gsistat # nco suggested
      if [ ! -f $INFILE ]; then
            echo -e "\n\n  INFILE:  $INFILE  does not exist \n\n"
            break
      fi
...
#      grep "type   gps jiter   3" $INFILE >> tmp
      ...
      cp tmpa ${SATCOMOUT_dir}/core_gsistat.${PDY}.${d_cyc}
      echo COUNTS  created ${SATCOMOUT_dir}/core_gsistat.${PDY}.${d_cyc}
    done 
