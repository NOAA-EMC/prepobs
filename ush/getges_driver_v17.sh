...
bn_COMINgdas=`basename $COMINgdas` # defined in obsproc/jobs/JOBSPROC_*PREP* or overwritten in triggers 
bn_COMINgfs=`basename $COMINgfs`   # defined in obsproc/jobs/JOBSPROC_*PREP* or overwritten in triggers 
                                   # export COMINgdas=${COMINgdas:-$(compath.py ${envir_getges}/gfs/${gfs_ver})/gdas.$PDY/${cyc}/${COMPONENT}} 
                                   # /lfs/h2/emc/gfstemp/emc.global/comroot/retrov17_01_realtime/${JTYP}.${PDY}/${cyc} #for now!!! IG /${COMPONENT} 
                                   # COMPONENT=obs in triggers jglobal*.wc2.obs.GFS17
				   # !!!for now!!!See "GFSv17 Downstream Dependency..." document Cathy/Jessica
#######################################################################################################
#######################################################################################################
if [[ `echo $bn_COMINgdas | cut -c1` = g && `echo $bn_COMINgfs | cut -c1` = g ]];then #base w.out path starts with "g", i.e. doesnt end on /atmos
...
#######################################################################################################
#######################################################################################################
elif [[ `echo $bn_COMINgdas | cut -c1` != g && `echo $bn_COMINgfs | cut -c1` != g ]];then  # base doesn't start with "g"
...
if [[ `echo $bn_COMINgdas | cut -c1` = a && `echo $bn_COMINgfs | cut -c1` = a ]];then #check if last part of path start with "a" (/atmos)
...
if [[ `echo $bn_COMINgdas | cut -c1` = a && `echo $bn_COMINgfs | cut -c1` = a ]];then # path includes /atmos
   ...
   export COMINgdas=$COMINgdas_root/gdas.${PDY_this_try}/${cyc_this_try} # path  doesn't include /atmos
...
if [[ `echo $bn_COMINgdas | cut -c1` = a && `echo $bn_COMINgfs | cut -c1` = a ]];then # path include /atmos
   # if atmos suffix, define $COMINgdas and $COMINgfs accordingly 
   export COMINgdas=$COMINgdas_root/gdas.${PDY_this_try}/${cyc_this_try} #/${COMPONENT}
   export COMINgfs=$COMINgfs_root/gfs.${PDY_this_try}/${cyc_this_try} #/${COMPONENT}
else
