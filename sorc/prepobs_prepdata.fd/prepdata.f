...
C      PARAMETER (NUMVAR = 6, NUMQMS = 5, NUMGPS = 3)
      PARAMETER (NUMVAR = 9, NUMQMS = 5)
C PARAMETER NAME "NUMDAT" THROUGHOUT PGM SETS SIZE OF DAT AND TMP ARRAYS
C      PARAMETER (NUMDAT = NUMVAR + (2 * NUMQMS) + NUMGPS)
...
C      EQUIVALENCE  (DAT(1,NUMVAR+NUMQMS+1+NUMGPS),IR)
...
C  Something Values:
C   DAT(NL,12) - HRDR   DAT(NL,13) - XDR   DAT(NL,14) - YDR
C
...
C      II = IDATA(14) - 14
      NN = MIN(NN,LEVPM)
      PRINT *, "NICKE i13 i14-11=ii NN", IDATA(13),IDATA(14),II,NN
      IZBEST = 99
      IF(NN.LE.0)  GO TO 11
      DO N = 1,NN
         II = II + 11
...
C GET PREPBUFR TABLE VALUES FOR PRESSURE, HGHT, TEMP., MOISTURE, & WIND
         ...
         PRINT *, "NICKE ADPUPA1:",RDATA(II+1),RDATA(II+2),RDATA(II+3)
         PRINT *, "NICKE ADPUPA2:",RDATA(II+4),RDATA(II+5),RDATA(II+6)
         PRINT *, "NICKE ADPUPA3:",RDATA(II+7),RDATA(II+8),RDATA(II+9)
         PRINT *, "NICKE ADPUPA4:",IPM,IZM,ITM,IQM,IVM
         PRINT *, "NICKE ADPUPA5:",RDATA(II+10),RDATA(II+11)
         PRINT *, "NICKE ADPUPA6:",RDATA(II+12),RDATA(II+13)
         PRINT *, "NICKE ADPUPA7:",RDATA(II+14),RDATA(II+15)
         PRINT *, "NICKE ADPUPA8:",RDATA(II+16),RDATA(II+17)
         PRINT *, "NICKE ADPUPA9:",RDATA(II+18),RDATA(II+19)
         PRINT *, "NICKE ADPUPA10:",RDATA(II+20),RDATA(II+21)
         PRINT *, "NICKE ADPUPA11:",RDATA(II+22),RDATA(II+23)
         PRINT *, "NICKE ADPUPA12:",RDATA(II+24),RDATA(II+25)
         IBTEST = 0
...
C PROCESS NON-MSG WIND REGARDLESS OF PREPBUFR TABLE VALUE
            ...
            IQ(NL,4) = IVM
         END IF
C         IF(RDATA(II+12).LT.YMISS) THEN
C            DAT(NL,12) = NINT(RDATA(II+12))
C         END IF
C         IF(RDATA(II+13).LT.YMISS) THEN
C            DAT(NL,13) = (NINT(RDATA(II+13))/100000.)
C         END IF
C         IF(RDATA(II+14).LT.YMISS) THEN
C            DAT(NL,14) = (NINT(RDATA(II+14))/100000.)
C         END IF
      ENDDO
   11 CONTINUE
      NM = NL
...
C NB IS NO. OF BUFR DATA LEVELS (CAT. 10) ACTUALLY COLLECTED
      PRINT *, "NICKE IDATA"
      DO QQQ = 1,100
         PRINT *, "QQQ: ", QQQ, IDATA(QQQ), RDATA(QQQ)
      ENDDO
      NB = 0
      NN = IDATA(31)
      II = IDATA(32) - 11
...
C x   NN = MIN(NN,LEVPM)
      ...
      DO N = 1,NN
         PRINT *, "NICKE PRE", RDATA(II+12),RDATA(II+13),RDATA(II+14)
         II = II + 11
C         II = II + 14
C GET PREPBUFR TABLE VALUES FOR PRESSURE, HGHT, TEMP., MOISTURE, & WIND
         IPM = NINT(RDATA(II+ 6))
...
C PROCESS NON-MSG WIND REGARDLESS OF PREPBUFR TABLE VALUE
         ...
         IF(RDATA(II+12).LT.BMISS) THEN
            DAT(NL,12) = RDATA(II+12)
            IS(NL,1) = RDATA(II+12)
         END IF
         IF(RDATA(II+13).LT.BMISS) THEN
             DAT(NL,13) = (NINT(RDATA(II+13))*0.00001)
             IS(NL,2) = (NINT(RDATA(II+13))*0.00001)
         END IF
         IF(RDATA(II+14).LT.BMISS) THEN
             DAT(NL,14) = (NINT(RDATA(II+14))*0.00001)
             IS(NL,3) = (NINT(RDATA(II+14))*0.00001)
         END IF
         PRINT *, "NICKE NEWRDATA1a", DAT(NL,1),DAT(NL,2),DAT(NL,3)
         PRINT *, "NICKENEWRDATA",RDATA(II),RDATA(II+1),RDATA(II+2) 
         PRINT *, "NICKE NEWRDATA2a", DAT(NL,4),DAT(NL,5),DAT(NL,6)
         PRINT *, "NICKENEWRDATA",RDATA(II+3),RDATA(II+4),RDATA(II+5)
         PRINT *, "NICKE NEWRDATA3a", DAT(NL,7),DAT(NL,8),DAT(NL,9)
         PRINT *, "NICKENEWRDATA",RDATA(II+6),RDATA(II+7),RDATA(II+8) 
         PRINT *, "NICKE NEWRDATA4a", DAT(NL,10),DAT(NL,11),DAT(NL,12)
         PRINT *, "NICKENEWRDATA",RDATA(II+9),RDATA(II+10),RDATA(II+11)
         PRINT *, "NICKE NEWRDATA5a", DAT(NL,13),DAT(NL,14),DAT(NL,15)
         PRINT *, "NICKENEWRDATA",RDATA(II+12),RDATA(II+13),RDATA(II+14)
         PRINT *, "NICKE NEWRDATA6a", DAT(NL,16),DAT(NL,17),DAT(NL,18)
         PRINT *, "NICKENEWRDATA",RDATA(II+15),RDATA(II+16),RDATA(II+17)
         PRINT *, "NICKE NEWRDATA7a", DAT(NL,19),DAT(NL,20),DAT(NL,21)
         PRINT *, "NICKENEWRDATA",RDATA(II+18),RDATA(II+19),RDATA(II+20)
         PRINT *, "NICKE 345: ",RDATA(II+12),RDATA(II+13),RDATA(II+14)
         PRINT *, "NICKE IS1: ", IS(NL,1),IS(NL,2),IS(NL,3)
...
C      PARAMETER (MAXOBS = 3500)
     ...
     $ (IDATA,RDATA)
      DATA   XMISS/99999./,YMISS/99998.8/,IMISS/99999/
      EQUIVALENCE  (DAT(1,NUMVAR+NUMQMS+1+NUMGPS),IR)
C FCNS BELOW CONVERT TEMP/TD (K) & PRESS (MB) INTO SAT./ SPEC. HUM.(G/G)
      ES(T) = 6.1078 * EXP((17.269 * (T - 273.16))/((T - 273.16)+237.3))
      QFRMTP(T,P) = (0.622 * ES(T))/(P - (0.378 * ES(T)))
      IF(IPRT(2).EQ.1)  THEN
         ...
         PRINT'(1X)'
      END IF
      PRINT *, "NICKE STOROB NUMDAT NUMVAR", NUMDAT, NUMVAR
      LL_MASS=0
      LL_WIND=0
...
C  MNEMONICS PASSED INTO SUBROUTINE "UFBINT".
     ...
     $ .FALSE.,3*.TRUE.,.FALSE.,2*.TRUE.,.FALSE./,ITYPL/0/,ISUB/0/,
     $ ISUBT/0/,IRECL/0/,AERS/12*0.0/
      DATA YMISS/99998.8/,IMISS/99999/
