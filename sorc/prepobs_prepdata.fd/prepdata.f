...
C DATA AT PRESSURE LEVELS (MESHED SOUNDING)
      ...
      EQUIVALENCE  (DAT(1,NUMVAR+NUMQMS+NUMGPS+1),IR)
      DATA  XMISS/99999./,YMISS/99998.8/,IMISS/99999/
...
C NM IS NO. OF MANDATORY DATA LEVELS (CAT. 1) ACTUALLY COLLECTED
      ...
      II = IDATA(14) - 11
      NN = MIN(NN,LEVPM)
C      PRINT *, 'nn=idata13,idata14,ii=idata14,nn',IDATA(13),IDATA(14),II,NN
      IZBEST = 99
      IF(NN.LE.0)  GO TO 11
      DO N = 1,NN
         II = II + 11
...
C PROCESS NON-MSG WIND REGARDLESS OF PREPBUFR TABLE VALUE
            ...
            IQ(NL,4) = IVM
         END IF
         IF(RDATA(II+12).LT.YMISS) THEN
            DAT(NL,12) = NINT(RDATA(II+12))
         END IF
         IF(RDATA(II+13).LT.YMISS) THEN
            DAT(NL,13) = (NINT(RDATA(II+13))/100000.)
         END IF
         IF(RDATA(II+14).LT.YMISS) THEN
            DAT(NL,14) = (NINT(RDATA(II+14))/100000.)
         END IF
         PRINT *, "NICKE NEW RDATA",DAT(NL,12),DAT(NL,13),DAT(NL,14)
      ENDDO
...
C$$$
     ...
     $ TDRIFT_LL(MXLVL),KFLAG(5),LFLAG(3),PLO,IFLTIM,JJ,II
      EQUIVALENCE  (DAT,PP),(DAT(1,2),ZP),(DAT(1,3),TP),(DAT(1,4),DP),
     $ (DAT(1,5),UP),(DAT(1,6),VP),(DAT(1,NUMVAR+1),IQ),(DAT(1,12),XP),
     $ (DAT(1,13),YP),(DAT(1,14),HP),
     $ (IDATA,RDATA)
      DATA   XMISS/99999./,YMISS/99998.8/
      DATA   PI/3.141592654/
